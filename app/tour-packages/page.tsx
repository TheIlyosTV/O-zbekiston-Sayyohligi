...
const tourPackages = [
  {
    ...
    duration: "10 kun",
    price: 1500,
    description: "Samarqand, Buxoro va Xiva shaharlarini o'z ichiga olgan klassik tur",
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {tourPackages.map((pack) => (
          <div key={pack.id} className="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <Image src={pack.image} alt={pack.name} fill className="object-cover rounded-t-lg" />
            </div>
            <div className="p-4">
              <h2 className="text-xl font-semibold mb-2">{pack.name}</h2>
              <p className="text-gray-600 mb-2">{pack.description}</p>
              <p className="text-gray-800 font-semibold mb-2">Davomiyligi: {pack.duration}</p>
              <p className="text-accent font-bold mb-4">Narxi: ${pack.price}</p>
              <Link 
                href={`/payment?amount=${pack.price}&package=${pack.id}`}
